...
void cmdNick(Server &irc, Message *message, int sender)
{
	if (message->get_parameters().size() == 1)
	{
		...
		{
			for (int i = 0; i < irc.pollfds.size(); i++)
			{
				if (irc.pollfds[i].fd == sender)
					continue;
				if (irc.getNickByFd(irc.pollfds[i].fd) == nick)
				{
					std::string join = ":server 433 * " + irc.getNickByFd(sender) + " :Nickname is already in use\n";
					logConsole(join);
					send(sender, join.c_str(), join.length(), MSG_DONTWAIT);
					close(sender);
					return;
		...
		{
			std::string join = ":server 433 * " + irc.getNickByFd(sender) + " :Nickname is already set\n";
			logConsole(join);
			send(sender, join.c_str(), join.length(), MSG_DONTWAIT);
		}
	}
	...
	{
		std::string join = ":server 431 * :No nickname given\n";
		logConsole(join);
		send(sender, join.c_str(), join.length(), MSG_DONTWAIT);
	}
}
...
void cmdPart(Server &irc, Message *message, int sender)
{
	...
	std::string msg = ":" + irc.getNickByFd(sender) + "!" + irc.clients[sender].getUsername() + "@" + irc.getHostname() + " PART " + chn + " " + reason;
